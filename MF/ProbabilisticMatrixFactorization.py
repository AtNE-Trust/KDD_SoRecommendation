...
class PMF(object):
    ...
    def fit(self, train_vec, test_vec):
        ...
        pairs_train = train_vec.shape[0]  # train_data 中条目数
        pairs_test = test_vec.shape[0]  # test_data中条目数

        # 1-p-i, 2-m-c
        num_user = int(max(np.amax(train_vec[:, 0]), np.amax(test_vec[:, 0]))) + 1  # 第0列，user总数
        num_item = int(max(np.amax(train_vec[:, 1]), np.amax(test_vec[:, 1]))) + 1  # 第1列，item总数

        incremental = False  # 增量
        if ((not incremental) or (self.w_Item is None)):
            # initialize 初始化
            self.epoch = 0
            self.w_Item = 0.1 * np.random.randn(num_item, self.num_feat)  # numpy.random.randn item M x D 正态分布矩阵
            self.w_User = 0.1 * np.random.randn(num_user, self.num_feat)  # numpy.random.randn 用户 N x D 正态分布矩阵

            self.w_Item_inc = np.zeros((num_item, self.num_feat))  # 创建item M x D 0矩阵
        ...
        while self.epoch < self.maxepoch:  # 检查迭代次数
            ...
            # Batch update
            for batch in range(self.num_batches):  # 每次迭代要使用的数据量 10
                #print ("epoch %d batch %d" % (self.epoch, batch+1))

                test = np.arange(self.batch_size * batch, self.batch_size * (batch + 1))#0-batch_size 
        ...
        # 将由PMF 得到的user，item latent feature存到TXT文件中，模型中load。
        
        user_PMF_data = self.w_User
        fw_user = open('user_PMF_train_data.txt','wb')
        pickle.dump(user_PMF_data, fw_user, -1)
        fw_user.close()
        
        item_PMF_data = self.w_Item
        fw_item = open('item_PMF_train_data.txt','wb')
        pickle.dump(item_PMF_data, fw_item, -1)
        fw_item.close()
